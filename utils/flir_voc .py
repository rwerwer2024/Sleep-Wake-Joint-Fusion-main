...
def parse_voc_annotation(data_path, file_type, anno_path, use_difficult_bbox=False):
    ...
    with open(anno_path, 'a') as f:
        for image_id in tqdm(image_ids):
            image_id = image_id.split("_PreviewData")[0]
            image_path = os.path.join(data_path, 'vi', image_id  + '.jpeg')
...
if __name__ =="__main__":
    ...
    train_data_path_2012 = "D:/align_flir"
    # train_data_path_2012 = cfg.DATA_PATH
    train_annotation_path = os.path.join('../data', 'test_annotation.txt')
    if os.path.exists(train_annotation_path):
        os.remove(train_annotation_path)

    # val_set   : VOC2007_test
    # test_data_path_2007 = os.path.join(cfg.DATA_PATH, 'kitti_voc_type')
    # # test_data_path_2007 = cfg.DATA_PATH
    # test_annotation_path = os.path.join('../data', 'test_annotation.txt')
    # if os.path.exists(test_annotation_path):
    #     os.remove(test_annotation_path)

    len_train = parse_voc_annotation(train_data_path_2012, "test", train_annotation_path, use_difficult_bbox=False)
