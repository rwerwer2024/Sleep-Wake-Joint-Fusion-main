...
class RandomCrop_new(object):
    ...
    def __call__(self, img, bboxes,img_shap_org):
        if random.random() < self.p:
            h_img_nw, w_img_nw, _ = img.shape
            h_img, w_img = img_shap_org[0],img_shap_org[1]
            ...
            #向上取整 ceil  向下取整int()
            import math
            crop_ymin = math.ceil((crop_ymin/h_img)*h_img_nw)
            crop_ymax = max(h_img, int((crop_ymax/h_img)*h_img_nw))
            crop_xmin = math.ceil((crop_xmin / w_img) * w_img_nw)
            crop_xmax = max(w_img, int((crop_xmax / w_img) * w_img_nw))
...
class Mixup_torch(object):
    ...
    def __call__(self, img_org, bboxes_org, img_mix, bboxes_mix):
        if random.random() > self.p:
            ...
            bboxes_org = torch.cat(
                [bboxes_org, torch.full((len(bboxes_org), 1), lam).cuda()], axis=1)
            bboxes_mix = torch.cat(
                [bboxes_mix, torch.full((len(bboxes_mix), 1), 1 - lam).cuda()], axis=1)
            bboxes =torch.cat([bboxes_org, bboxes_mix])

        else:
            img = img_org
            bboxes = torch.cat([bboxes_org, torch.full((len(bboxes_org), 1), 1.0).cuda()], axis=1)
